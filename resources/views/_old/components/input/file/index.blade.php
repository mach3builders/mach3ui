...
@php
    ...
    $displayClasses = Ui::classes()
        ->add('block h-10 min-w-0 flex-1 truncate rounded-s-md border border-r-0 px-3 py-2 text-sm')
        ->add('border-gray-140 bg-white text-gray-500')
        ->add('dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400');

    $buttonClasses = Ui::classes()
        ->add(
            'btn inline-flex h-10 shrink-0 cursor-pointer select-none items-center justify-center gap-2 whitespace-nowrap rounded-e-md border px-4 text-xs font-semibold uppercase shadow-xs',
        )
        ->add('border-gray-120 bg-white text-gray-700 hover:bg-gray-20 hover:text-gray-900')
        ->add(
            'dark:border-gray-690 dark:bg-gray-800 dark:text-gray-100 dark:hover:bg-gray-780 dark:hover:text-gray-20',
        );
@endphp
...
<div x-data="{
    ...
    <span class="{{ $displayClasses }}" x-text="displayText"></span>

    <span class="{{ $buttonClasses }}">{{ $resolvedButtonText }}</span>

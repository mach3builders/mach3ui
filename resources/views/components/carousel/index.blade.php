    ...
    @if ($controls)
        <div class="pointer-events-none absolute inset-y-0 right-0 left-0 flex items-center justify-between">
            <button
                ...
                :disabled="current === 0"
                @class([
                    'pointer-events-auto inline-flex size-8 -translate-x-1/2 cursor-pointer items-center justify-center rounded-full border shadow-sm transition-colors',
                    'border-gray-200 bg-white/80 text-gray-700 backdrop-blur-sm hover:bg-white hover:text-gray-900',
        ...
        <div class="flex justify-center gap-1.5 pt-4">
            <template x-for="i in total" :key="i">
                <button
                    ...
                    @click="goTo(i - 1)"
                    :class="current === i - 1
                        ? 'size-2 rounded-full bg-gray-900 dark:bg-gray-100'
                        : 'size-2 rounded-full bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600'"
                ></button>
            </template>
        </div>
    @endif

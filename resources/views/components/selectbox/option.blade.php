...
@php
    $classes = Ui::classes()
        ->add('flex w-full cursor-pointer items-center gap-2 rounded-md px-2.5 py-2 text-left text-sm transition-colors')
...
<button
    ...
    x-bind:aria-selected="isSelected({{ $jsValue }})"
    data-value="{{ $value }}"
    @if ($disabled) disabled aria-disabled="true" @endif
    {{ $attributes->except('class') }}
    class="{{ $classes }}"
    data-selectbox-option
>

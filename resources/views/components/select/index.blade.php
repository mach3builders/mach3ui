...
@php
    ...
    $selectClasses = Ui::classes()
        ...
        ->add("bg-[url('data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2216%22%20height%3D%2216%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22%239ca3af%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%3E%3Cpath%20d%3D%22m6%209%206%206%206-6%22%2F%3E%3C%2Fsvg%3E')]")
        ->add($size, [
...
@if ($label)
    <ui:field :id="$id">
        ...
        <x-ui::select._select :id="$id" :name="$selectName" :error="$error"
            :placeholder="$placeholder" :select-classes="$selectClasses"
            :attributes="$attributes">{{ $slot }}</x-ui::select._select>

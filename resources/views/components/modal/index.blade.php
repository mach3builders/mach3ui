...
@php
    ...
    $panelClasses = Ui::classes()
        ...
        ->add(
            'backdrop:bg-gray-900/50 backdrop:backdrop-blur-sm backdrop:opacity-0 backdrop:pointer-events-none backdrop:transition-all backdrop:duration-200 backdrop:ease-out',
        )
...
<dialog id="{{ $modalId }}" wire:ignore.self x-modelable="open" x-data="{
    open: {{ $openState }},
    init() {
        const id = this.$el.id;

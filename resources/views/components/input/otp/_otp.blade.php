...
<div x-data="{
    ...
    }" x-modelable="value" {{ $attributes->whereStartsWith('x-model') }} class="{{ $wrapperClasses }}"
    {{ $attributes->only('data-*') }} data-input-otp data-control @if ($error) aria-invalid="true" @endif>

    <input type="hidden" x-ref="hidden" id="{{ $id }}" @if ($name) name="{{ $name }}" @endif
        ...
        @foreach ($groups as $groupIndex => $group)
            <div class="flex items-center" data-input-otp-group>
                @foreach ($group as $index)
                    <div class="{{ $slotClasses }}" :class="{ '{{ $activeRingClasses }}': activeIndex === {{ $index }} }"
    ...
    @else
        <div class="flex items-center" data-input-otp-group>
            @for ($i = 0; $i < $length; $i++)
                <div class="{{ $slotClasses }}" :class="{ '{{ $activeRingClasses }}': activeIndex === {{ $i }} }"

...
@php
    ...
    $safeAttributes = $attributes->filter(fn ($value) => is_string($value) || is_bool($value) || is_numeric($value) || is_null($value));

    $containerClasses = Ui::classes()
        ->add('relative w-full')
        ->merge($safeAttributes);
...
>
    <canvas
        @if($height) style="height: {{ is_numeric($height) ? $height . 'px' : $height }}" @endif
    ></canvas>
</div>
